...
2. 生成多个候选实现到 `lora_workspace/candidates/`，不要直接覆盖 best。
3. 使用 `evaluate_lora_candidate` 做 correctness + benchmark 评测。
4. 至少比较多个候选，再决定是否用 `promote_lora_candidate` 晋升为新的 `optimized_lora.cu`；严禁晋升未通过 correctness 的候选。
5. 搜索时先用 `quick` 预设快速筛选；在准备收尾前，必须对当前 best 至少做一次 `full` 预设验证。
6. 如有必要，可使用 phase1 的 probe 工具辅助判断瓶颈，但不要把 profiling 当作主链。
7. 每轮最多做一个小步改动并记录意图，例如“替换 B^T X 路径”“尝试 block size 变体”“尝试轻量 fusion”，避免同时修改太多点导致无法归因。
